...
import {
  ...
  ScreenshotModal, IndicatorSettingModal, SymbolSearchModal, OverlayToolbar,
  OverlaySettingsModal
} from './widget'

import { getOverlaySettings } from './widget/overlay-settings-modal/registry'
import type { OverlaySettingsConfig, OverlaySettingsValues } from './widget/overlay-settings-modal/types'
...
const ChartProComponent: Component<ChartProComponentProps> = props => {
  ...
  const [overlayToolbar, setOverlayToolbar] = createSignal<{
    visible: boolean, overlayId: string,
    isLocked: boolean, x: number, y: number
  }>({ visible: false, overlayId: '', isLocked: false, x: 0, y: 0 })

  let toolbarCustomPos: { x: number, y: number } | null = null

  const [overlaySettingsModal, setOverlaySettingsModal] = createSignal<{
    visible: boolean
    overlayId: string
    overlayName: string
    config: OverlaySettingsConfig | null
    currentValues: OverlaySettingsValues
  }>({ visible: false, overlayId: '', overlayName: '', config: null, currentValues: {} })
  ...
  return (
    <>
      ...
      {/* intentionally empty — toolbar renders inside chart content */}
      <PeriodBar
        locale={props.locale}
        symbol={symbol()}
      ...
      <div
        ...
        <Show when={drawingBarVisible()}>
          <DrawingBar
            ...
            onDrawingItemClick={overlay => {
              setOverlayToolbar(prev => ({ ...prev, visible: false }))
              widget?.createOverlay({
                ...overlay,
                onMouseEnter: (event) => {
                  const mainDom = widget?.getDom('candle_pane', DomPosition.Main) as HTMLElement | null
                  if (mainDom) mainDom.style.cursor = 'pointer'
                  const o = event.overlay
                  const ext = o.extendData ?? {}
                  if (!ext._hovered) {
                    widget?.overrideOverlay({ id: o.id, extendData: { ...ext, _hovered: true } })
                  }
                  return false
                },
                onMouseLeave: (event) => {
                  const mainDom = widget?.getDom('candle_pane', DomPosition.Main) as HTMLElement | null
                  if (mainDom) mainDom.style.cursor = 'crosshair'
                  const o = event.overlay
                  const ext = o.extendData ?? {}
                  if (ext._hovered) {
                    widget?.overrideOverlay({ id: o.id, extendData: { ...ext, _hovered: false } })
                  }
                  return false
                },
                onSelected: (event) => {
                  const o = event.overlay
                  const ext = o.extendData ?? {}
                  widget?.overrideOverlay({ id: o.id, extendData: { ...ext, _selected: true } })
                  let tx: number, ty: number
                  if (toolbarCustomPos) {
                    tx = toolbarCustomPos.x
                    ty = toolbarCustomPos.y
                  } else {
                    const rect = contentRef?.getBoundingClientRect()
                    tx = (event.pageX ?? event.x ?? 0) - (rect?.left ?? 0)
                    ty = Math.max(0, ((event.pageY ?? event.y ?? 0) - (rect?.top ?? 0)) - 44)
                  }
                  setOverlayToolbar({
                    visible: true,
                    overlayId: o.id,
                ...
                onDeselected: (event) => {
                  const o = event.overlay
                  const ext = o.extendData ?? {}
                  widget?.overrideOverlay({ id: o.id, extendData: { ...ext, _selected: false } })
                  setOverlayToolbar(prev => ({ ...prev, visible: false }))
                  return false
                }
              })
        ...
        <Show when={overlayToolbar().visible}>
          <OverlayToolbar
            locale={props.locale}
            overlayId={overlayToolbar().overlayId}
            isLocked={overlayToolbar().isLocked}
            x={overlayToolbar().x}
            y={overlayToolbar().y}
            onLock={(id, lock) => { widget?.overrideOverlay({ id, lock }) }}
            onRemove={(id) => {
            ...
            onSettings={(id) => {
              ...
              const config = getOverlaySettings(name)
              const currentValues = overlay.extendData?.settings ?? {}
              setOverlaySettingsModal({
                visible: true,
                overlayId: id,
                overlayName: name,
                config,
            ...
            onClose={() => { setOverlayToolbar(prev => ({ ...prev, visible: false })) }}
            onPositionChange={(x, y) => { toolbarCustomPos = { x, y } }}
          />
        </Show>
        <Show when={overlaySettingsModal().visible}>
          <OverlaySettingsModal
            locale={locale()}
            overlayName={overlaySettingsModal().overlayName}
            config={overlaySettingsModal().config ?? { inputs: [], styles: [] }}
            currentValues={overlaySettingsModal().currentValues}
            onConfirm={(values) => {
              const id = overlaySettingsModal().overlayId
              const overlay = widget?.getOverlayById(id)
              const existingExtend = overlay?.extendData ?? {}
              widget?.overrideOverlay({ id, extendData: { ...existingExtend, settings: values } })
            }}
            onClose={() => { setOverlaySettingsModal(prev => ({ ...prev, visible: false })) }}
          />
        </Show>
      </div>
    </>
